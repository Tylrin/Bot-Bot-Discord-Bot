...
module.exports.run = async (client, message, arguments) => {
    ...
    if (!roleUser.roles.has(guildRole.id)) { // Check if the have the role.
        message.reply("They already have that role.")
        return;
    } 
    await (roleUser.addRole(guildRole.id)); // Add the role.
